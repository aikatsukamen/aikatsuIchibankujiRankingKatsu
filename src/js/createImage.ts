 ...
 */
export const createRankingImage = async (rankList: RankList): Promise<string> => {
  /** 背景画像 */
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (rank.rank > 8) break; // なぜかうっかりTOP10じゃないやつが混じらないように
    // 順位
  ...
  logger.system.info(`[createRankingImage]${fileImagePath}`);
  return fileImagePath;
};

const idolIconPath = (idolname: string): string => {
  const idolMap = {
    '♥ 星宮いちご': 1,
    '♥ 北大路さくら': 2,
    '♥ 姫里マリア': 3,
...
const compareRank = (a: RankList['all'][0], b: RankList['all'][0]) => {
